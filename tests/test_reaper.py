...
@pytest.mark.noparallel(groups=[NoParallelGroups.XRD])
@pytest.mark.parametrize("caches_mock", [{"caches_to_mock": [
    'rucio.daemons.reaper.reaper.REGION'
]}], indirect=True)
def test_deletion_with_tokens(vo, did_factory, root_account, caches_mock):
    rse_name = 'XRD4'
    rse_id = rse_core.get_rse_id(rse=rse_name, vo=vo)
    ...
    reaper(once=True, rses=[], include_rses=rse_name, exclude_rses=None, chunk_size=1000, greedy=True)
