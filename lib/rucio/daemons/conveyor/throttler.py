...
from rucio.core.request import cancel_requests, get_request_stats, get_requests_to_cancel, re_sync_all_transfer_limits, release_all_waiting_requests, release_waiting_requests_fifo, release_waiting_requests_grouped_fifo, reset_stale_waiting_requests, set_transfer_limit_stats
...
def throttler(
    ...
    def _consumer(release_groups: Optional["ReleaseGroupsDict"]) -> None:
        ...
        if requests_to_cancel:
            cancelled = cancel_requests([r['request_id'] for r in requests_to_cancel])
            logger(logging.INFO, "Throttler - cancelled %d waiting requests", cancelled)
